    ...
    <main className="min-h-screen bg-imo-deep text-white font-body overflow-x-hidden">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 pb-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sc.work.map((item) => (
            <article key={item.title} className="glass-card p-6 group text-center">
              <span className="text-4xl block mb-4 group-hover:scale-110 transition-transform duration-300" aria-hidden="true">
                {item.icon}
              </span>
              <h2 className="text-sm font-heading tracking-wider text-white mb-3 group-hover:text-imo-sky transition-colors">
