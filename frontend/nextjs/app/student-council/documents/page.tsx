  ...
  if (type === 'pdf') {
    return (
      <svg className="w-6 h-6 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21h10a2 2 0 002-2V9l-6-6H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
          ...
          {sc.documents.map((doc) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-sm font-medium text-white group-hover:text-imo-sky transition-colors truncate">
                  {doc.title}
                </h2>
                <p className="text-xs text-white/40 mt-0.5 uppercase tracking-wide">{doc.type}</p>
              </div>
              <svg className="w-5 h-5 text-white/20 group-hover:text-imo-sky transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
